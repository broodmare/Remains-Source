package interdata 
{
	...
	import stubs.butStand;
	import stubs.visualStandInfo;
	import stubs.itemArt;
	import stubs.visBodyStay;
	...
	public class Stand 
	{
		...
		public function createWeaponLists(n:int):void
		{
		...
		public function createArtList(n:int):void
		{
		...
		public function createArmorList(n:int):void
		{
			var stolb:int 	= 0;
			var str:int 	= 0;
		...
		public function showWeapon(item:MovieClip, n:int, respect:int):void
		{
			if (n == 0) 
			{
				...
				item.objectName.visible=item.objectName2.visible=false;
			...
			else if (n == 1) 
			{
				...
				item.objectName2.visible=false;
				item.weapon.visible=true;
				item.weapon2.visible=false;
			...
			if (item.objectName.visible || item.objectName2.visible) 
			{
				if (respect == 1) 
				{
		...
		public function itemOver(event:MouseEvent):void
		{
			...
			if (!event.currentTarget.objectName.visible && !event.currentTarget.objectName2.visible) return;
			info.objectName.text=event.currentTarget.objectName.visible?event.currentTarget.objectName.text:event.currentTarget.objectName2.text;
			if (event.currentTarget.objectName2.visible) info.info.htmlText=PipPage.infoStr(Item.L_WEAPON,event.currentTarget.id.text+'^'+inv.weapons[event.currentTarget.id.text].variant);
