...
//============================================================================================		
		...
		/* ****** */World.w.gr_stage=3;
			...
			if (frontBitmap) visFront.removeChild(frontBitmap);
			frontBmp=new BitmapData(resX, resY, true, 0x0)
			frontBitmap = new Bitmap(frontBmp);
			visFront.addChild(frontBitmap);
			
			if (backBmp) backBmp.dispose();
			if (backBitmap) visBack.removeChild(backBitmap);
			backBmp=new BitmapData(resX, resY, true, 0x0)
			backBitmap = new Bitmap(backBmp);
			visBack.addChild(backBitmap);
			
			if (backBmp2) backBmp2.dispose();
			if (backBitmap2) visBack2.removeChild(backBitmap2);
			backBmp2=new BitmapData(resX, resY, true, 0x0)
			backBitmap2 = new Bitmap(backBmp2);
			visBack2.addChild(backBitmap2);

			if (vodaBmp) vodaBmp.dispose();
			if (vodaBitmap) visVoda.removeChild(vodaBitmap);
			vodaBmp=new BitmapData(resX, resY, true, 0x0)
			vodaBitmap = new Bitmap(vodaBmp);
			visVoda.addChild(vodaBitmap);
			
			if (lightBmp) lightBmp.dispose();
			if (lightBitmap) visLight.removeChild(lightBitmap);
			lightBmp=new BitmapData(loc.spaceX+1, loc.spaceY+3,true,0xFF000000);
			...
			if (satsBmp) satsBmp.dispose();
			if (satsBitmap) visSats.removeChild(satsBitmap);
			satsBmp=new BitmapData(resX, resY,true,0);
