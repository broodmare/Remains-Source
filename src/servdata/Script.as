	...
	public class Script {
		...
		function com(obj:Object) {
			...
			if (obj.targ) {
				...
				if (obj.act=='allact') {
					World.w.location.allAct(null, obj.val, obj.n);
				}
				if (obj.act=='take') {
					if (obj.n<0) {
				...
				if (obj.act=='locon') {
					World.w.location.allon();
				}
				if (obj.act=='locoff') {
					World.w.location.alloff();
				}
				if (obj.act=='quest') {
				...
				if (obj.act=='actprob') {
					if (World.w.location.prob) World.w.location.prob.activateProb();
				}
				if (obj.act=='alarm') {
					World.w.location.signal();
				}
				if (obj.act=='trus') {
					if (owner && owner.location) owner.location.trus=Number(obj.val);
					else World.w.location.trus=Number(obj.val);
				}
				if (obj.act=='checkall') {
				...
				if (obj.act=='robots') {
					World.w.location.robocellActivate();
				}
				if (obj.act=='weapch') {
				...
				if (obj.act=='wave') {
					if (World.w.location.prob) World.w.location.prob.beginWave();
				}
				if (obj.act=='pip') {
