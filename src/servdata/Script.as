		...
		public function step() {
			...
			if (tcom<=0) {
				if (wait) {
					...
					World.world.ctr.keyPressed=World.world.ctr.keyPressed2=false;
				}
				ncom++;
				if (ncom>=acts.length) {
					running=false;
		...
		function com(obj:Object) {
			...
			if (obj.targ) {
				...
				if (obj.act=='take') {
					if (obj.n<0) {
						if (World.world.invent.items[obj.val]) {
							World.world.gui.infoText('withdraw',World.world.invent.items[obj.val].nazv, -obj.n);
				...
				if (obj.act=='gotoland') {	//перейти в местность
					...
					else if (obj.n==1) World.world.game.gotoLand(obj.val, obj.opt1+':'+obj.opt2);
